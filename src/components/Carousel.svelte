<script>
  ...
  export let data = [{
    name: 1,
    // title: 'Missguided',
    logoImg: 'https://stripe.com/img/v3/payments/overview/logos/missguided.svg',
    img: 'https://stripe.com/img/v3/payments/overview/photos/missguided.jpg',
    desc: '<p class="text-gray-600"><span class="text-gray-900">Missguided</span> is a UK-based fashion retailer that has nearly doubled in size since last year. They integrated Stripe to deliver seamless checkout across mobile and web for customers in 100+ countries, all while automatically combating fraud.</p>'
  },{
    name: 2,
    logoImg: 'https://stripe.com/img/v3/payments/overview/logos/slack.svg',
    img: `https://picsum.photos/seed/${random_seed()}/352/160`,
    desc: '<p class="text-gray-600"><span class="text-gray-900">Missguided</span> is a UK-based fashion retailer that has nearly doubled in size since last year. They integrated Stripe to deliver seamless checkout across mobile and web for customers in 100+ countries, all while automatically combating fraud.</p>'
  },{
    name: 3,
    logoImg: 'https://stripe.com/img/v3/payments/overview/logos/charity_water.svg',
    img: `https://picsum.photos/seed/${random_seed()}/352/160`,
    desc: '<p class="text-gray-600"><span class="text-gray-900">Missguided</span> is a UK-based fashion retailer that has nearly doubled in size since last year. They integrated Stripe to deliver seamless checkout across mobile and web for customers in 100+ countries, all while automatically combating fraud.</p>'
  }]
  ...
  function leftClick() {
    current_idx = current_idx > 0 ? current_idx-1 : data.length - 1
  }
  function rightClick() {
    current_idx = current_idx < data.length - 1 ? current_idx+1 : 0
  }
</script>

<!-- Carousel Body -->
<div class="relative rounded-lg block md_flex items-center bg-gray-100 shadow-xl" style="min-height: 19rem;">
{#each [data[current_idx]] as item ( item.name )}
  <div in:fade={{duration: 500}} class="relative w-full md_w-2/5 h-full overflow-hidden rounded-t-lg md_rounded-t-none md_rounded-l-lg" style="min-height: 19rem;">
...
{/each}
  ...
  <button class="absolute top-0 mt-32 left-0 bg-white rounded-full shadow-md h-12 w-12 text-2xl text-indigo-600 hover_text-indigo-400 focus_text-indigo-400 -ml-6 focus_outline-none focus_shadow-outline" on:click={leftClick}>
    <span class="block" style="transform: scale(-1);">&#x279c;</span>
  </button>
  <button class="absolute top-0 mt-32 right-0 bg-white rounded-full shadow-md h-12 w-12 text-2xl text-indigo-600 hover_text-indigo-400 focus_text-indigo-400 -mr-6 focus_outline-none focus_shadow-outline"  on:click={rightClick}>
    <span class="block" style="transform: scale(1);">&#x279c;</span>
  </button>
</div>
...
<div class="flex items-center pt-5 justify-between">
  {#each data as item, i ( item.name )}
    <button class="px-2 opacity-50 hover_opacity-100 focus_opacity-100" on:click={()=> current_idx = i}>
      {#if item.logoImg}
        <img class="w-full" src="{item.logoImg}" alt="" style="max-height: 60px;">
      {:else}
        <svg width="239" height="120" viewBox="0 0 239 120" class="h-24 w-full font-xl" style="max-height: 60px;">
          <text x="0" y="80" font-size="300%" font-weight="800">{item.title}</text>
        </svg>
      {/if}
    </button>
  {/each}
<!--   
  <button class="px-2 opacity-50 hover_opacity-100 focus_opacity-100"><img class="w-full" src="https://stripe.com/img/v3/payments/overview/logos/slack.svg" alt="" style="max-height: 60px;"></button>
  <button class="px-2 opacity-50 hover_opacity-100 focus_opacity-100"><img class="w-full" src="https://stripe.com/img/v3/payments/overview/logos/glossier.svg" alt="" style="max-height: 60px;"></button>
  <button class="px-2 opacity-50 hover_opacity-100 focus_opacity-100"><img class="w-full" src="https://stripe.com/img/v3/payments/overview/logos/charity_water.svg" alt="" style="max-height: 60px;"></button>
  <button class="px-2 opacity-100 hover_opacity-100 focus_opacity-100"><img class="w-full" src="https://stripe.com/img/v3/payments/overview/logos/missguided.svg" alt="" style="max-height: 60px;"></button> -->
</div>