...
<!-- Carousel Body -->
<div
  class="relative rounded-lg block md:(flex items-center) bg-gray-100 shadow-xl min-h-19rem"
>
  {#each [data[current_idx]] as item (item.name)}
    <div
      in:fade={{ duration: 500 }}
      class="relative w-full md:w-2/5 h-full overflow-hidden rounded-t-lg md:(rounded-t-none rounded-l-lg) min-h-19rem"
    >
      <img
      ...
      >
        ...
        {:else}
          <svg
            ...
            height="120"
            viewBox="0 0 239 120"
            class="h-24 w-full text-xl"
          >
            <text x="0" y="80" font-size="300%" font-weight="800"
              >{item.title}</text
            >
          </svg>
        {/if}
    ...
    >
      <div class="p-12 md:(pr-24 pl-16 py-12)">
        {@html item.desc}
        <a
