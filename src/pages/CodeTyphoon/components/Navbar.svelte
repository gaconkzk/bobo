...
<nav class="flex-initial flex items-center justify-between flex-wrap p-6 border-opacity-75 shadow-lg naviga z-5">
  ...
  <div class="w-8 h-8 relative mb" on:click={avatarClick}>
    <div class="group w-full h-full rounded-full overflow-hidden shadow-inner text-center bg-purple table cursor-pointer">
      <span class="hidden group-hover_table-cell rounded-full text-white font-bold align-middle bg-gray-400 select-none">{shortName(user.name)}</span>
      <img src={user.avatar} alt="lovely avatar"
        class="object-fill object-center w-full h-8 visible group-hover_hidden" />
    </div>
{#if menuDisplay}
    <div transition:fade={{duration: 100}} class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg z-10">
