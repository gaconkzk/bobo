...
>
  <div
    ...
    style="background-image: url(https://picsum.photos/seed/{random_seed}/352/160)"
  />
  ...
  <div class="bg-white -mt-20 shadow-lg rounded-lg overflow-hidden p-4 w-5/6">
    <div class="header-content inline-flex items-center">
      ...
      <div class="category-title flex-1 text-sm mr-2">
        <i class="devicon {lang()}" />
      </div>
      <div class="title-post">title {data}</div>
    </div>

    <div
      class="summary-post text-base text-justify font-small max-h-6 pb-2 block-with-text"
    >
      <span class="text-sm italic normal"
    ...
    <button
      class="bg-green-500 text-white mt-4 block rounded py-1 px-2 text-sm "
      on:click={() => navigateTo(`/ct/${data}`)}
    >
